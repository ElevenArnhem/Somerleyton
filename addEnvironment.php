        ...
        <div class="col-lg-4">
            ...
            <form action="index.php?page=addEnvironment" method="post">
                <dl class="dl-horizontal">
                    <dt>Naam</dt>
                    <dd>
                        <input name="ENVIRONMENTNAME" type="text" class="form-control" value="';
                            if (isset($_POST['ENVIRONMENT']) || isset($_POST['ENVIRONMENTNAME'])) {
                                echo $environment;
                            }
                        echo '" placeholder="omgeving naam" required>
                    </dd><br>
                </dl>
                <input name="OLDENVIRONMENTNAME" type="hidden"  value="';
                    if (isset($_POST['ENVIRONMENT']) || isset($_POST['ENVIRONMENTNAME'])) {
                        echo $environment;
                    }
                    echo '" >
                <button class="btn btn-primary" name="TYPE" value="';
                    if (isset($_POST['ENVIRONMENT'])) {
                        echo 'changeEnvironment';
                    } else {
                        echo 'addEnvironment';
                    }
                    echo '" type="submit">Opslaan
                </button>';
                if (isset($_POST['ENVIRONMENT']) || isset($_POST['ENVIRONMENTNAME'])) {
                    echo '<button class="btn btn-primary" name="TYPE" value="deleteEnvironment" type="submit">Verwijder omgeving</button>';
                }
                echo '
            </form>
        </div>';
